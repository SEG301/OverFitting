...
### 4.2. Cấu trúc dữ liệu Inverted Index tạm (trong RAM)

```python
# Cấu trúc trong bộ nhớ mỗi block:
# term -> { doc_id -> term_frequency }
dictionary = defaultdict(lambda: defaultdict(int))

# Sau khi SPIMI-Invert:
# term -> [(doc_id_1, tf_1), (doc_id_2, tf_2), ...]
# Đã sắp xếp theo doc_id để chuẩn bị cho merge
```

### 4.3. Quy trình chi tiết (file `src/indexer/spimi.py`)
...
### 4.4. Kỹ thuật Boosting trong Indexing
